...
export default function LoginForm() {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="w-full px-8 py-12 my-6 mx-2 rounded-lg flex flex-col items-center-safe justify-center-safe gap-3" >
            ...
            <button disabled={isSubmitting} type="submit" className="bttn trnsition">{isSubmitting ? "Login in..." : "Login"}</button>
            <button disabled={isSubmitting} type="button" onClick={handleGoogleLogin} className="bttn-outw hover:text-gray-500 trnsition my-1 flex items-center-safe gap-2"><FcGoogle />Log in with Google</button>
        </form>
    )
}